...
int fContador;  //Variável utilizada para armazenar a quantidade de elementos na Fila
...
void Fila_Construtor()
{
  ...
  fContador = 0;   //Armazena a quantidade de elementos na Fila
...
bool Fila_Vazia()
{
   ...
   return Frente == NULL;  //Pela forma como os elementos são Enfileirados e Desenfileirados (métodos a seguir)
                          ...
                          //Esse comando poderia também ser substituído por fContador == 0
...
int Fila_Tamanho()
{
    return fContador;  //É retornado o fContador de elementos, manipulado nos métodos de Enfileirar e Desenfileirar
}
...
bool Fila_Enfileirar(Aluno Elemento)
{
   ...
   else
   {
          ...
          fContador++;       //Um elemento foi enfileirado, portanto deve-se incrementar o fContador de elementos da Fila
...
bool Fila_Desenfileirar(Aluno &Elemento)
{
     ...
     if(!Fila_Vazia()){  //Se a Fila não estiver vazia, podemos remover algum elemento
        ...
        fContador--;  //"fContador" é decrementado para indicar que um elemento foi removido da Fila (desenfileirado)
...
bool Fila_Frente(Aluno &Elemento)
{
